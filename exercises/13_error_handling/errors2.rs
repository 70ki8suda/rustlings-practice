...
fn main() {
    ...
    let quantities = vec!["10", "20", "thirty", "40"];

    for quantity in quantities {
        match total_cost(quantity) {
            Ok(cost) => println!("The total cost for {} items is {} tokens.", quantity, cost),
            Err(e) => println!("Failed to calculate total cost for '{}': {}", quantity, e),
        }
    }
}
